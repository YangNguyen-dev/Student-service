...
export default function StudentProfile() {
  ...
  if (loading) return <Box sx={{ display: 'flex', justifyContent: 'center', p: 6 }}><CircularProgress sx={{ color: '#6C63FF' }} /></Box>;

  return (
    <Box sx={{ animation: 'fadeIn 0.4s ease-out', '@keyframes fadeIn': { from: { opacity: 0 }, to: { opacity: 1 } }, maxWidth: 600, mx: 'auto' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 3 }}>
        <Avatar sx={{ backgroundColor: '#6C63FF22', color: '#6C63FF', width: 48, height: 48 }}><PersonRounded /></Avatar>
        <Box>
      ...
      {/* Avatar Section */}
      <Card sx={{ borderRadius: 3, background: 'linear-gradient(135deg, rgba(108,99,255,0.08) 0%, rgba(255,255,255,0.8) 100%)', border: '1px solid rgba(108,99,255,0.15)', mb: 3 }}>
        <CardContent sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', py: 4 }}>
          ...
          >
            <Avatar
              ...
              sx={{
                ...
                background: avatarSrc ? 'transparent' : 'linear-gradient(135deg, #6C63FF, #9D97FF)',
          ...
          <Button
            ...
            onClick={() => fileInputRef.current?.click()}
            sx={{ mt: 1.5, color: '#6C63FF', textTransform: 'none', fontSize: '0.8rem', '&:hover': { backgroundColor: 'rgba(108,99,255,0.08)' } }}
          >
      ...
      {/* Info Card */}
      <Card sx={{ borderRadius: 3, background: 'linear-gradient(135deg, rgba(108,99,255,0.05) 0%, rgba(255,255,255,0.8) 100%)', border: '1px solid rgba(108,99,255,0.15)' }}>
        <CardContent sx={{ p: 4 }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
            ...
            {!editing ? (
              <Button startIcon={<EditRounded />} onClick={() => setEditing(true)}
                sx={{ color: '#6C63FF', textTransform: 'none', '&:hover': { backgroundColor: 'rgba(108,99,255,0.08)' } }}>
                Chỉnh sửa
              </Button>
            ) : (
              <Box sx={{ display: 'flex', gap: 1 }}>
                ...
                <Button startIcon={<SaveRounded />} onClick={handleSave} variant="contained" size="small"
                  disabled={saving} sx={{ textTransform: 'none', backgroundColor: '#6C63FF' }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
            <Box>
              <Typography variant="caption" sx={{ color: 'text.secondary', mb: 0.5, display: 'block' }}>Mã học sinh</Typography>
              <Typography sx={{ color: 'text.primary' }}>{profile?.studentCode || '—'}</Typography>
            </Box>
            <Box>
            ...
            {editing ? (
              ...
              <>
                ...
                <Box>
                  <Typography variant="caption" sx={{ color: 'text.secondary', mb: 0.5, display: 'block' }}>Lớp</Typography>
                  <Typography sx={{ color: 'text.primary' }}>{profile?.classroomName || 'Chưa có lớp'}</Typography>
                </Box>
              </>
            )}
          </Box>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 2.5 }}>
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  setError('');
                  try {
                    await studentService.changePassword({ currentPassword: pwForm.currentPassword, newPassword: pwForm.newPassword });
